...
rm -rf /etc/fail2ban/
(cd ~/sources/mailcow/fail2ban/inst/0.9.3 ; python setup.py -q install)
...
if [ ${USE_MAILSERVER} == '1' ]; then
	systemctl -q restart {fail2ban,rsyslog,nginx,php7-fpm,spamassassin,dovecot,postfix,opendkim,clamav-daemon,fuglu,mailgraph}
else
	systemctl -q restart {fail2ban,nginx,php7-fpm}
fi
