...
DEBIAN_FRONTEND=noninteractive apt-get --force-yes -y install dovecot-common dovecot-core dovecot-imapd dovecot-lmtpd dovecot-managesieved dovecot-sieve dovecot-mysql dovecot-pop3d dovecot-solr >/dev/null
	...
	#ssl
	mkdir /etc/ssl/mail >>/root/stderror.log 2>&1  >> /root/stdout.log
	...
	if [[ ${ip_useable} -ne 1 ]]; then
		...
		tar xf letsencrypt-sh/inst/${letsencrypt_sh_version}.tar -C letsencrypt-sh/inst/ >>/root/stderror.log 2>&1  >> /root/stdout.log
	...
	tar xf fuglu/inst/${fuglu_version}.tar -C fuglu/inst/ >>/root/stderror.log 2>&1  >> /root/stdout.log
	...
	# Solr
	#if [[ -z $(curl -s --connect-timeout 3 "http://127.0.0.1:8983/solr/admin/info/system" >>/root/stderror.log 2>&1  >> /root/stdout.log | grep -o '[0-9.]*' | grep "^${solr_version}\$") ]]; then
	#	(
	#	TMPSOLR=$(mktemp -d)
	#	cd $TMPSOLR
