...
DEBIAN_FRONTEND=noninteractive ${APT} -y install dovecot-common dovecot-core dovecot-imapd dovecot-lmtpd dovecot-managesieved dovecot-sieve dovecot-mysql dovecot-pop3d dovecot-solr >/dev/null
		...
		#fuglu
			...
			tar xf fuglu/inst/0.6.6.tar -C fuglu/inst/ 2> /dev/null
			(cd fuglu/inst/0.6.6 ; python setup.py -q install)
			cp -R fuglu/conf/* /etc/fuglu/
			if [[ -f /lib/systemd/systemd ]]; then
				cp fuglu/inst/0.6.6/scripts/startscripts/debian/8/fuglu.service /etc/systemd/system/fuglu.service
				systemctl disable fuglu
				[[ -f /lib/systemd/system/fuglu.service ]] && rm /lib/systemd/system/fuglu.service
				systemctl daemon-reload
				systemctl enable fuglu
			else
				install -m 755 fuglu/inst/0.6.6/scripts/startscripts/debian/7/fuglu /etc/init.d/fuglu
		...
		#roundcube
			...
			tar xf roundcube/inst/1.2.1.tar -C roundcube/inst/
			cp -R roundcube/inst/1.2.1/* /var/www/mail/rc/
			if [[ ${is_upgradetask} != "yes" ]]; then
				...
				chmod +x roundcube/inst/1.2.1/bin/installto.sh
				roundcube/inst/1.2.1/bin/installto.sh /var/www/mail/rc
			fi
