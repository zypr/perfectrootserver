...
if [ ${USE_MAILSERVER} == '1' ]; then
  if [ ${MAILCOW_ADMIN_PASS} == 'generatepw' ]; then
	   MAILCOW_ADMIN_PASS=$passwordgenerator
...
if [ ${USE_PMA="1"} == '1' ]; then
  if [ ${PMA_HTTPAUTH_PASS} == 'generatepw' ]; then
	   PMA_HTTPAUTH_PASS=$passwordgenerator
  ...
  if [ ${PMA_BFSECURE_PASS} == 'generatepw' ]; then
	   PMA_BFSECURE_PASS=$passwordgenerator
...
if [ ${MYSQL_ROOT_PASS} == 'generatepw' ]; then
	   MYSQL_ROOT_PASS=$passwordgenerator
...
if [ ${MYSQL_MCDB_PASS} == 'generatepw' ]; then
	   MYSQL_MCDB_PASS=$passwordgenerator
...
if [ ${MYSQL_RCDB_PASS} == 'generatepw' ]; then
	   MYSQL_RCDB_PASS=$passwordgenerator
...
if [ ${MYSQL_PMADB_PASS} == 'generatepw' ]; then
	   MYSQL_PMADB_PASS=$passwordgenerator
